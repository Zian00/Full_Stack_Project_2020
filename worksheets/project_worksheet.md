...
-   Q1. Consider the use case of `Compute basic/advance result`, **HOW** is this functionality made available to both JiBaBoom and Adminstrator _(Hint: They are different)_? `(Jibaboom viewed the result via API in backend, Administrator views it in the frontend?)`
-   Q2. Consider the use case of `Insert basic/advance data`. **HOW** is the data stored? `(The data is stored in the cloud database (ElephantSql))`
...
3. How should the frontend display data from the database? `It communicates to the backend API, which will then make a http request to the database via a database driver and the databse will then service the response, then pass it back to the backend, which will then pass it to the frontend.`
4. How does the frontend communicate with backend? `the frontend uses an api to communicate with the backend to service requests the client needs.`
...
```
how can frontend/mobile get data from backend?
```

Think about how you can do it and type your answer below:

```
the frontend gets data from the backend via GET API requests
---
